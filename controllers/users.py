...
async def create_user( user: User ) -> User:
    ...
    try:
        ...
        if "email already exists" in str(e).lower():
            raise HTTPException(status_code=400, detail="El correo ya está registrado en Firebase.")
        raise HTTPException(status_code=500, detail=f"Error al registrar usuario en Firebase: {str(e)}")
...
async def login(user : Login) -> dict:
    ...
    coll = get_collection("users")
    user_info = coll.find_one({ "email": user.email.lower() })
